...
object Toon {
  ...
  def random: Toon = {
    val clazzes = permittedCombos.map(_._2).distinct
    val clazz = choose(clazzes)
    val races = permittedCombos.filter(_._2 == clazz).map(_._1).distinct
    val race = choose(races)
    ...
    Toon("", race, clazz, gender, city, belief)
  }
  ...
  private val permittedCombos: Seq[(Race, Clazz, Belief, City)] = List(
    ...
    (Halfling, Paladin, Karana, Rivervale),
    (Halfling, Ranger, Karana, Rivervale),
    ...
    (Gnome, Paladin, BrellSerilis, AkAnon),
    (Gnome, ShadowKnight, Bertoxxulous, AkAnon),
